...
const DashBoardmap = () => {
  ...
  const handleSourceSelect = (airport) => {
    if (airport && airport.coordinates) {
      ...
      console.log("Source selected:", airport.coordinates);
    } else {
  ...
  const handleDestinationSelect = (airport) => {
    if (airport && airport.coordinates) {
      setDestination({ lat: airport.coordinates.lat, lng: airport.coordinates.lon });
      console.log("Destination selected:", airport.coordinates);
    } else {
      console.error("Invalid destination selection");
    }
  };
  ...
  const printCoordinates = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/get_paths', {
      ...
      alert("Error fetching paths. Please try again.");
    }
  };
  ...
  const fetchLiveFlights = async () => {
    try {
      const options = {
        ...
        headers: {
          "X-RapidAPI-Key": "1bf12c4288msh35ce70e76276336p1c28dajsn6ea40c431385",
      ...
      <div className="input-container">
        ...
        <div className="legends-container">
          <div className="legend" style={{ marginTop: '4px'}}>
            <img
              src="https://www.freeiconspng.com/uploads/airplane-icon--transport-iconset--icons-land-5.png"
